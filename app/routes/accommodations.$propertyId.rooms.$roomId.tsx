...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  // Get availability calendar (3 months)
  const availabilityCalendar = await getRoomAvailabilityCalendar(roomId, new Date(), 3);
  ...
  if (checkIn && checkOut) {
    ...
    if (!isNaN(checkInDate.getTime()) && !isNaN(checkOutDate.getTime()) && checkOutDate > checkInDate) {
      ...
      dateRangeInfo = await checkDateRangeAvailability(
        roomId,
        checkInDate,
        checkOutDate,
        1 // numberOfRooms
...
export default function RoomDetail() {
  const { property, room, user, availabilityCalendar, dateRangeInfo, pricingInfo, searchParams } = useLoaderData<typeof loader>();
  ...
  const handleBookNow = () => {
    ...
    if (!user) {
      navigate(`/login?redirectTo=/accommodations/${property.id}/rooms/${room.id}?${urlSearchParams.toString()}`);
      return;
    }
                ...
                <div className="mb-6 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">
                      {room.currency} {room.basePrice.toLocaleString()} × {searchParams.numberOfNights} night{searchParams.numberOfNights > 1 ? 's' : ''}
                    </span>
                    <span className="font-medium">{room.currency} {pricingInfo.basePrice.toLocaleString()}</span>
                  </div>
                  {pricingInfo.cleaningFee > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Cleaning fee</span>
                      <span className="font-medium">{room.currency} {pricingInfo.cleaningFee.toLocaleString()}</span>
                    </div>
                  )}
                  {pricingInfo.serviceFee > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Service fee</span>
                      <span className="font-medium">{room.currency} {pricingInfo.serviceFee.toLocaleString()}</span>
                    </div>
                  )}
                  {pricingInfo.taxes > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Taxes</span>
                      <span className="font-medium">{room.currency} {pricingInfo.taxes.toLocaleString()}</span>
                    </div>
                  )}
